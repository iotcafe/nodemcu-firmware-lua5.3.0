...
static const char *pushnexttemplate (lua_State *L, const char *path) {
  ...
  l = c_strchr(path, *LUA_PATH_SEP);  /* find next separator */
...
#else
  return checkload(L, (luaL_loadfsfile(L, filename) == LUA_OK), filename);
#endif
}
...
static int loadfunc (lua_State *L, const char *filename, const char *modname) {
  ...
  mark = c_strchr(modname, *LUA_IGMARK);
  if (mark) {
    int stat;
...
static void setpath (lua_State *L, const char *fieldname, const char *envname1,
  ...
  const char *path = c_getenv(envname1);
  if (path == NULL)  /* no environment variable? */
    path = c_getenv(envname2);  /* try alternative name */
