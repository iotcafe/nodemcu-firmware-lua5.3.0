...
static int str_rep (lua_State *L) {
  ...
  if (n <= 0) lua_pushliteral(L, "");
    ...
    while (n-- > 1) {  /* first n-1 copies (followed by separator) */
      c_memcpy(p, s, l * sizeof(char)); p += l;
      if (lsep > 0) {  /* empty 'memcpy' is not that cheap */
        c_memcpy(p, sep, lsep * sizeof(char));
        p += lsep;
      }
    }
    c_memcpy(p, s, l * sizeof(char));  /* last copy (not followed by separator) */
...
static int nospecials (const char *p, size_t l) {
  size_t upto = 0;
  do {
    if (c_strpbrk(p + upto, SPECIALS))
      return 0;  /* pattern has a special character */
    upto += c_strlen(p + upto) + 1;  /* may have more after \0 */
  } while (upto <= l);
...
static const char *scanformat (lua_State *L, const char *strfrmt, char *form) {
  ...
  *(form++) = '%';
  c_memcpy(form, strfrmt, (p - strfrmt + 1) * sizeof(char));
  form += p - strfrmt + 1;
  *form = '\0';
  return p;
}
  ...
  while (*fmt != '\0') {
    ...
    switch (opt) {
      ...
      case Kzstr: {  /* zero-terminated string */
        size_t len;
        const char *s = luaL_checklstring(L, arg, &len);
        luaL_argcheck(L, c_strlen(s) == len, arg, "string contains zeros");
        luaL_addlstring(&b, s, len);
...
static int str_unpack (lua_State *L) {
  ...
  while (*fmt != '\0') {
    ...
    switch (opt) {
      ...
      case Kzstr: {
        size_t len = (int)c_strlen(data + pos);
