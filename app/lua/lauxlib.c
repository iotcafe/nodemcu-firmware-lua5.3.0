...
static int pushglobalfuncname (lua_State *L, lua_Debug *ar) {
  ...
  if (findfield(L, top + 1, 2)) {
    ...
    if (c_strncmp(name, "_G.", 3) == 0) {  /* name start with '_G.'? */
...
LUALIB_API int luaL_fileresult (lua_State *L, int stat, const char *fname) {
  ...
  if (stat) {
    ...
    lua_pushnil(L);
    if (fname)
      lua_pushfstring(L, "%s: %s", fname, c_strerror(en));
    else
      lua_pushstring(L, c_strerror(en));
...
LUALIB_API char *luaL_prepbuffsize (luaL_Buffer *B, size_t sz) {
  ...
  if (B->size - B->n < sz) {  /* not enough space? */
    ...
    c_memcpy(newbuff, B->b, B->n * sizeof(char));
    if (buffonstack(B))
...
LUALIB_API int luaL_loadfsfilex (lua_State *L, const char *filename, const char *mode) {
  LoadFSF lf;
  int status, readstatus;
  int c;
  int fnameindex = lua_gettop(L) + 1;  /* index of filename on the stack */
  ...
  fs_ungetc(c, lf.f);
  status = lua_load(L, getFSF, &lf, lua_tostring(L, -1), mode);
...
static void *l_alloc (void *ud, void *ptr, size_t osize, size_t nsize) {
  ...
  if (nsize == 0) {
    ...
    return (void *)c_realloc(ptr, nsize);
}
